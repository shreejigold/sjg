...
export default function ProductsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let result;
    if (isEditing && editId) {
      result = await ProductService.updateProduct(editId, productPayload);
    } else {
      result = await ProductService.createProduct(productPayload);
    }

    if (result.success) {
      setSuccessMsg(isEditing ? "Masterpiece refined!" : "Asset logged in vault.");
      resetForm();
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Securely dispose of this asset?")) return;
    ...
    <div className="min-h-screen bg-softgray/50 text-darkbrown font-montserrat flex overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar bg-white/50">
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md px-6 md:px-10 py-6 border-b border-gold/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-playfair font-bold text-darkbrown">Asset Custodian</h1>
            <p className="text-[10px] uppercase tracking-[0.2em] text-zinc-500 mt-1">Global Inventory Architecture</p>
          </div>
          ...
          <section className="space-y-8">
            <div className="glass-card rounded-[3rem] border border-gold/10 p-10 md:p-16 relative overflow-hidden shadow-2xl shadow-pink/5 bg-white">
              <div className="absolute -top-10 -right-10 opacity-[0.03] rotate-12">
                <Hash size={340} className="text-gold" />
              </div>
              ...
              <div className="flex items-center justify-between mb-12 relative z-10">
                <h2 className="text-3xl font-playfair font-bold text-gold flex items-center gap-4">
                  ...
                  {isEditing ? "Refine Masterpiece" : "Log New Inventory"}
                </h2>
                {isEditing && (
              ...
              <form onSubmit={handleSubmit} className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16">
                <div className="space-y-10">
                  <div className="space-y-3">
                    <label className="text-[10px] uppercase tracking-[0.3em] text-zinc-500 font-bold ml-2">Identity Title</label>
                    <input 
                      type="text" 
                      placeholder="e.g. Maharaja Gold Crown Piece" 
                      value={title}
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    <div className="space-y-3">
                      ...
                      <input type="number" value={mrp || ""} onChange={(e) => setMrp(Number(e.target.value))} className="w-full bg-white border border-gold/10 rounded-2xl py-5 px-4 text-center font-bold text-xl" />
                    </div>
                    <div className="space-y-3">
                    ...
                    <div className="space-y-3">
                      <label className="text-[10px] uppercase tracking-[0.3em] text-gold font-bold ml-2 text-center block">Final Price</label>
                  ...
                  <div className="grid grid-cols-2 gap-8">
                     <div className="space-y-3">
                        <label className="text-[10px] uppercase tracking-[0.3em] text-zinc-500 font-bold ml-2">Collection Class</label>
                        <select 
                ...
                <div className="space-y-10">
                   <div className="space-y-3">
                      <label className="text-[10px] uppercase tracking-[0.3em] text-zinc-500 font-bold ml-2 flex justify-between">
                         Visual Assets <span className="text-zinc-400">{images.length}/3 Limit</span>
                      </label>
                      <div className="grid grid-cols-3 gap-6">
                         {images.map((img, idx) => (
                         ...
                         {images.length < 3 && (
                           <button 
                             type="button" 
                             onClick={() => document.getElementById('prodInp')?.click()}
                  ...
                  <div className="flex items-center gap-6 pt-6">
                      ...
                      >
                        <span className="text-[10px] font-black uppercase tracking-widest">{hide ? 'Hidden' : 'On Live Store'}</span>
                      ...
                      >
                        {isLoading ? <Loader2 className="animate-spin" size={28} /> : (
                          isEditing ? <>Refinement Complete <CheckCircle2 size={24} /></> : <>Authorize Inventory <CheckCircle2 size={24} /></>
                        )}
                      </button>
                   </div>
          ...
          <section className="space-y-12 pb-20">
             <div className="flex flex-col md:flex-row items-center justify-between gap-8 border-b border-gold/10 pb-10">
                <div className="text-center md:text-left">
                   <h2 className="text-3xl font-playfair font-bold text-darkbrown uppercase tracking-tighter italic">Live Repository</h2>
                   <p className="text-[10px] text-zinc-400 font-black tracking-[0.3em] mt-2">ACCESS VAULT RECORDS BY CATEGORY</p>
                </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                ...
                {!hasSearched && (
                   <div className="col-span-full py-40 flex flex-col items-center justify-center border-2 border-dashed border-gold/10 rounded-[4rem] bg-softgray/20">
                      ...
                      <h3 className="text-2xl font-playfair font-black text-darkbrown/30 italic">Records Secured.</h3>
                      <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400 mt-4">Select collection filter to access inventory audit</p>
                   </div>
                )}
                ...
                {hasSearched && products.length === 0 && (
                  <div className="col-span-full py-40 flex flex-col items-center opacity-30 italic">
                     ...
                     <p className="text-sm tracking-widest uppercase font-black">No matching inventory records found.</p>
                  </div>
                )}
             </div>
          </section>
