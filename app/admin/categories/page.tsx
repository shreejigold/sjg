...
import { 
  ...
  CheckCircle2, User, EyeOff, Eye, Loader2, RefreshCw, Sparkles
} from "lucide-react";
        ...
        <div className="p-6 md:p-10 w-full max-w-[1400px] mx-auto space-y-16">
            ...
            <div className="glass-card rounded-[3rem] border border-gold/10 p-10 md:p-16 relative overflow-hidden shadow-2xl shadow-pink/5 bg-white">
              ...
              <h2 className="text-3xl font-playfair font-bold text-gold mb-12 flex items-center justify-between relative z-10">
                <div className="flex items-center gap-4">
                  ...
                  {isEditing ? "Refine Collection" : "Craft New Collection"}
                </div>
                {isEditing && (
                  ...
                  <div className="space-y-8 flex flex-col justify-between">
                    <div className="space-y-3">
                      <label className="text-[10px] uppercase tracking-[0.3em] text-zinc-500 font-bold ml-2 flex justify-between">
                         Visual Identity <span className="text-gold italic font-medium">Recommended: 1:1 Aspect</span>
                      </label>
                      <button 
                         type="button" 
                    ...
                    >
                      {isLoading ? <Loader2 className="animate-spin" size={28} /> : (
                        isEditing ? <>Authorize Updates <CheckCircle2 size={24} /></> : <>Initialize Collection <CheckCircle2 size={24} /></>
                      )}
                    </button>
                  </div>
                </div>
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {categories.map((cat) => (
                <div key={cat.id} className="glass-card p-6 rounded-[2.5rem] border border-gold/10 flex flex-col gap-6 hover:border-gold/30 transition-all duration-500 group bg-white shadow-xl shadow-pink/5">
                  <div className="relative w-full aspect-square rounded-[2rem] overflow-hidden border border-gold/5">
                    <img src={cat.image} alt={cat.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                    <div className="absolute top-4 right-4 flex gap-2">
                       {cat.trending && <div className="bg-gold/80 backdrop-blur-md text-white p-2 rounded-full shadow-lg"><Sparkles size={14} /></div>}
                       {cat.hide && <div className="bg-red-500/80 backdrop-blur-md text-white p-2 rounded-full shadow-lg"><EyeOff size={14} /></div>}
                    </div>
                  </div>

                  <div className="space-y-1 block text-center">
                    <h3 className="text-xl font-bold text-darkbrown group-hover:text-gold transition-colors">{cat.title}</h3>
                    <p className="text-[10px] text-zinc-400 uppercase tracking-widest">{cat.discount}% Promotional Discount</p>
                  </div>

                  <div className="flex items-center justify-center gap-3 pt-2">
                    <button 
                      onClick={() => startEdit(cat)}
                      className="p-3 bg-softgray hover:bg-gold hover:text-white rounded-2xl transition-all duration-500 text-darkbrown/40"
                    >
                       <RefreshCw size={18} />
                    </button>
                    <button 
                      onClick={() => toggleCategoryStatus(cat.id, 'trending', cat.trending)}
                      className={`flex-1 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all ${cat.trending ? 'bg-gold text-white shadow-lg' : 'bg-softgray text-darkbrown/40 border border-gold/10'}`}
                    >
                      {cat.trending ? 'Trending' : 'Standard'}
                    </button>
                    <button 
                      onClick={() => toggleCategoryStatus(cat.id, 'hide', cat.hide)}
                      className={`px-4 py-3 rounded-2xl transition-all ${cat.hide ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'}`}
                    >
                      {cat.hide ? <EyeOff size={18} /> : <Eye size={18} />}
                    </button>
                  </div>
                </div>
              ))}
