    ...
    <div className="min-h-screen bg-white text-darkbrown font-montserrat flex flex-col pt-32 relative">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 w-full py-12 md:py-20 lg:py-24">
        ...
        <div className="flex items-center justify-between mb-16 border-b border-gold/10 pb-10">
           <div>
             <h1 className="text-4xl md:text-6xl font-playfair font-bold text-darkbrown">Fulfillment Center</h1>
             <p className="text-[10px] uppercase tracking-[0.3em] text-gold font-bold mt-4 italic">Confirm your legacy shipping details</p>
           </div>
          ...
          <section className="space-y-12">
             <form onSubmit={handleSubmit} className="space-y-10">
                ...
                <div className="space-y-3 pt-4">
                    <label className="text-[10px] uppercase tracking-[0.3em] font-black text-zinc-400 ml-2">Payment Preference *</label>
                    <div className="grid grid-cols-2 gap-4">
                        {PAYMENT_METHODS.map((method) => (
                           <button
                             ...
                             className={`px-6 py-4 rounded-2xl border text-[11px] font-bold uppercase tracking-widest transition-all
                                ${paymentMethod === method ? 'bg-gold text-white border-gold shadow-lg shadow-gold/20' : 'bg-softgray/20 border-gold/5 text-darkbrown/40 hover:border-gold/30'}`}
                           >
                              {method}
                           </button>
                        ))}
                ...
                >
                  {isProcessing ? <><Loader2 className="animate-spin" size={24} /> Processing Legacy Request...</> : <>Confirm Order Now <ShieldCheck size={24} /></>}
                </button>
             </form>
          </section>
             ...
             <div className="glass-card rounded-[3rem] p-12 border border-gold/10 sticky top-40 bg-white/50 backdrop-blur-sm">
                ...
                <div className="space-y-6 pt-12 border-t border-gold/10">
                   <div className="flex justify-between items-center text-sm">
                      <span className="text-darkbrown/50 font-medium tracking-tight uppercase tracking-widest text-[10px]">Vault Subtotal</span>
                      <span className="font-bold text-lg">₹{subtotal.toLocaleString()}</span>
                   </div>
                   <div className="flex justify-between items-center text-sm">
                      <span className="text-darkbrown/50 font-medium tracking-tight uppercase tracking-widest text-[10px]">Insured Transit</span>
                      <span className="text-gold font-bold uppercase text-[10px] tracking-widest italic">Gratis</span>
                   </div>
                   <div className="flex justify-between items-center pt-8 border-t border-gold/10">
                      ...
                      <span className="text-4xl font-black text-darkbrown">₹{subtotal.toLocaleString()}</span>
                   </div>
                </div>
